...
if (isset($_GET['id'])){
    ...
    $product_brand_id = escape($row['product_brand_id']);
    ...
    $query = query("SELECT * FROM brands WHERE brand_id=" . $product_brand_id);
    confirm($query);
    $row = fetch_array($query);
...
<aside id="admin_sidebar" class="col-md-4">
    ...
    <div class="form-group">
        ...
        <option value="{$product_category_id}" selected>{$product_cat_title}</option>
...
DELIMITER;
    ...
    <div class="form-group">
        ...
        <option value="{$product_brand_id}" selected>{$product_brand_title}</option>
DELIMITER;
         echo $form;
        ...
        $query = query("SELECT * FROM brands");
         confirm($query);
         while($row = fetch_array($query)){
            if ($product_brand_id != $row['brand_id']){
...
<!-- Product Tags -->
    ...
    <!-- Product Image -->
    <img style="width:75px" src="{$product_image}" alt="">
    <div class="form-group">
